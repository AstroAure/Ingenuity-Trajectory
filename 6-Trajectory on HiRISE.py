...
def anim_trajectory_HiRise(background, images_filename, begin, end, ingenuity_template, num_targets, target_size, min_vector, point_template, ingenuity_marker):
    ...
    for i in range(1, len(POSITIONS_BG)):
        ...
        cv.line(background, POSITIONS_BG[i-1], POSITIONS_BG[i], WHITE, thickness = Wbg//500)
        ...
        cv.imwrite("6-Trajectory on HiRise\\" + str(i) + ".png", bg_copy)
...
IMAGES = anim_trajectory_HiRise(background, images_filename, begin, end, ingenuity_template, num_targets, target_size, min_vector, point_template, ingenuity_marker)

for i in range(len(IMAGES)):

    cv.imwrite("6-Trajectory on HiRise\\" + str(i) + ".png", IMAGES[i])
